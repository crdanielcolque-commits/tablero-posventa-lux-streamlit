...
# v2.3.13
# ✅ Lectura robusta de hojas (match “normalizado”)
# ✅ TAB: 🗓️ Hitos del mes desde hoja "Resumen del mes" (fuzzy match)
# ============================================================
...
def norm_text(x: str) -> str:
    ...
    s = (s.replace("á","a").replace("é","e").replace("í","i")
           ...
           .replace("ñ","n"))
    # normalizo separadores y espacios
...
def find_sheet_name(sheet_names, target: str):
    """
    Encuentra el nombre real de la hoja haciendo match por texto normalizado.
    target: por ejemplo "Resumen del mes" o "Dias habiles"
    """
    t = norm_text(target)
    ...
    for s in sheet_names:
        if norm_text(s) == t:
            return s
    # 2) match por contiene (por si viene como "Resumen del mes (2)" etc.)
    for s in sheet_names:
        if t in norm_text(s):
            return s
...
with tab1:
    ...
    st.info(
        f"**Resumen Ejecutivo:** "
        f"Repuestos {pct(rep_c)} | Servicios {pct(srv_c)} | Total Postventa {pct(total_c)} | {driver_txt}"
    )
...
# ============================================================
# TAB 2 — KPIs resto (se mantiene)
# ============================================================
with tab2:
    st.info("TAB 2 sin cambios en esta versión (se mantiene igual que tu v2.3.12).")

# ============================================================
# TAB 3 — Gestión (se mantiene)
# ============================================================
with tab3:
    st.info("TAB 3 sin cambios en esta versión (se mantiene igual que tu v2.3.12).")
...
with tab4:
    ...
    # Debug amable: qué hoja encontró
    if RESUMEN_SHEET_FOUND is None:
        ...
        with st.expander("Ver pestañas detectadas (para identificar el nombre exacto)", expanded=True):
            ...
            st.write("Tip: si tu hoja se llama por ejemplo 'Resumen del Mes' / 'Resumen_mes' / 'Resumen del mes ' (con espacio al final), ahora debería detectarla igual.")
        st.stop()
    ...
    if df_resumen_mes is None or df_resumen_mes.empty:
        ...
        if tmp.empty:
            ...
            # no asumo estructura: cada fila como “Col: Valor | Col: Valor”
